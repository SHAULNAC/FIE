...
let shouldClearSearchOnType = false;

searchInput.addEventListener('focus', () => {
    shouldClearSearchOnType = Boolean(searchInput.value);
});

searchInput.addEventListener('click', () => {
    if (!searchInput.value) return;
    searchInput.value = '';
    shouldClearSearchOnType = false;
});

searchInput.addEventListener('keydown', (e) => {
    if (shouldClearSearchOnType && e.key.length === 1 && !e.ctrlKey && !e.metaKey && !e.altKey) {
        searchInput.value = '';
        shouldClearSearchOnType = false;
    }
});
