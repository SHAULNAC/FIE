...
function renderSearchControls() {
    ...
    const modeToggle = `
        <div class="search-controls-top">
            <button class="search-chip ${playbackIsPlaylist ? 'active' : ''}" onclick="togglePlaybackMode()" title="החלף מצב הפעלה">
                <i class="fa-solid ${playbackIsPlaylist ? 'fa-list' : 'fa-wand-magic-sparkles'}"></i>
                <span>${modeLabel}</span>
            </button>
        </div>
    `;
...
async function applyChannelFilter(channelName) {
    ...
    if (!channelMatchResults.some(c => c.name.toLowerCase() === normalizedChannelName.toLowerCase())) {
